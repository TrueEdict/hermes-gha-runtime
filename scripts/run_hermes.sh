...
for key in OPENROUTER_API_KEY OPENAI_API_KEY ANTHROPIC_API_KEY DEEPSEEK_API_KEY NOUS_API_KEY OPENCODE_GO_API_KEY KAGGLE_USERNAME KAGGLE_KEY KAGGLE_API_TOKEN OPENCODE_AUTH_JSON_PRIMARY OPENCODE_AUTH_JSON_FALLBACK COLAB_ADC_JSON HF_TOKEN MODAL_TOKEN_ID MODAL_TOKEN_SECRET; do
  if [ -n "${!key:-}" ]; then
    docker_args+=( -e "$key" )
  fi
done
